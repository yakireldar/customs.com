...
# 1. הגדרות עיצוב ומראה נקי (NFX Style)
...
st.markdown("""
    ...
    .status-badge-free { background-color: #DCFCE7; color: #15803D; padding: 4px 10px; border-radius: 12px; font-size: 13px; font-weight: 600; }
    .status-badge-restrict { background-color: #FEE2E2; color: #B91C1C; padding: 4px 10px; border-radius: 12px; font-size: 13px; font-weight: 600; }
...
@st.cache_data(ttl=86400) # מטמון ל-24 שעות. האתר יתרענן אוטומטית פעם ביום!
def fetch_live_government_data():
    """ פונקציה הפונה ל-API הרשמי של דאטה גוב ומורידה את ספר המכס המעודכן """
    # מזהה המאגר הרשמי של ספר סיווג טובין ביבוא ברשות המסים
    ...
    try:
        ...
        # המרת הנתונים לטבלה נוחה לעבודה
        raw_df = pd.DataFrame(records)
        
        # מיפוי השדות האמיתיים מהממשלה לשדות של האתר שלנו (שמות השדות מותאמים למאגר הממשלתי)
        clean_data = []
        ...
        # במידה ואתר הממשלה קרס או חסם - נחזור לנתוני הגיבוי המקומיים כדי שהאתר לא ייפול
        fallback_data = [
...
# טעינת הנתונים (מתרענן אוטומטית פעם ביום או בלחיצת כפתור)
df, last_update_time = fetch_live_government_data()

# 3. מבנה האתר (UI)
st.markdown('<div class="nfx-title">NFX - מרכז המכס והסחר הבינלאומי</div>', unsafe_allowed_html=True)
st.markdown('<div class="nfx-tagline">מערכת חכמה לסיווג, בדיקת שיעורי מס וחוקיות יבוא (צו יבוא חופשי)</div>', unsafe_allowed_html=True)

# שורת סטטוס המציגה לגולש שהמידע אכן מעודכן להיום
st.markdown(f"""
...
if search_query:
    # סינון חכם לפי קוד או תיאור פריט
    filtered_df = df[df['code'].str.contains(search_query) | df['description'].str.contains(search_query, case=False)]
    
    if not filtered_df.empty:
        st.markdown(f"<p style='color: #64748B; font-size: 14px;'>נמצאו {len(filtered_df)} תוצאות מעודכנות:</p>", unsafe_allowed_html=True)
